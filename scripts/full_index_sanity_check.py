...
for q in queries:
    ...
    t0 = time.time(); np_res = npidx.search(qv, 10); np_times.append(time.time()-t0)
    t0 = time.time(); f_res = fidx.search(qv, 10); f_times.append(time.time()-t0)
    ...
    print(f"  numpy top1: {np_res[0][0]} score={np_res[0][1]:.4f} text={chunk_text[np_res[0][0]][:60]!r}")
    print(f"  faiss top1: {f_res[0][0]} score={f_res[0][1]:.4f} text={chunk_text[f_res[0][0]][:60]!r}")
